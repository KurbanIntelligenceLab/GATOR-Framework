...
def classify_adsorption_mode(r_hh: float) -> str:
    ...
    if r_hh < MODE_MOLECULAR_MAX:
        return 'molecular'
    elif MODE_ACTIVATED_MIN <= r_hh <= MODE_ACTIVATED_MAX:
        return 'activated'
    else:  # r_hh > MODE_DISSOCIATIVE_MIN
        return 'dissociative'


def load_energies_from_tab3(csv_filepath: str, system_name: str) -> Optional[Tuple[float, float, float]]:
    """
...
class AdsorptionMetrics:
    ...
    def __init__(self, xyz_filepath: str, material_name: str,
                 e_np_h2: Optional[float] = None,
                 e_np: Optional[float] = None,
                 e_h2: Optional[float] = None,
                 tab3_filepath: Optional[str] = None):
        """
    ...
    def _calculate_metrics(self):
        ...
        for i in range(len(h_indices)):
            for j in range(i + 1, len(h_indices)):
                dist = calculate_distance(
                    self.coordinates[h_indices[i]],
                    self.coordinates[h_indices[j]]
                )
        ...
        ae_indices = [i for i, elem in enumerate(self.elements) 
                     if elem in ae_elements]
        ...
        if len(ae_indices) > 0:
            ...
            for h_idx in [h1_idx, h2_idx]:
                for ae_idx in ae_indices:
                    dist = calculate_distance(
                        self.coordinates[h_idx],
                        self.coordinates[ae_idx]
                    )
                    if dist < min_h_ae_distance:
                        min_h_ae_distance = dist
            
            self.r_h_ae = min_h_ae_distance
            ...
            for o_idx in o_indices:
                for ae_idx in ae_indices:
                    dist = calculate_distance(
                        self.coordinates[o_idx],
                        self.coordinates[ae_idx]
                    )
                    if dist < min_o_ae_distance:
                        min_o_ae_distance = dist
            
            self.r_o_ae = min_o_ae_distance
        else:
    ...
    def __repr__(self) -> str:
        ...
        return (f"AdsorptionMetrics(material_name='{self.material_name}', "
...
def calculate_adsorption_metrics(xyz_filepath: str, material_name: str,
                                 e_np_h2: Optional[float] = None,
                                 e_np: Optional[float] = None,
                                 e_h2: Optional[float] = None,
                                 tab3_filepath: Optional[str] = None) -> Dict:
    """
